...
Page({
  ...
  async loadLedger() {
    ...
    try {
      ...
      if (!selected) {
        wx.showToast({ title: "账本不存在或无权访问", icon: "none" });
        wx.navigateBack();
        return;
      }
  ...
  manageMembers() {
    wx.showToast({ title: "成员管理入口已预留", icon: "none" });
  },
  async inviteLedger() {
    const ledger = this.data.ledger;
    if (!ledger || !ledger.isOwner) return;
    wx.showLoading({ title: "准备分享" });
    try {
      const shareOptions = await Promise.all(
        getShareOptions(ledger).map(async (item) => {
  ...
  stopSheetTap() {},
  selectShareMode(e) {
    this.setData({ shareMode: e.currentTarget.dataset.mode || "visitor" });
  },
  onShareAppMessage(options = {}) {
    ...
    const mode =
      ledger.type === "personal"
        ? "visitor"
        : (options.target && options.target.dataset.mode) || this.data.shareMode;
    const option =
      this.data.shareOptions.find((item) => item.mode === mode) || this.data.shareOptions[0] || {};
    return {
      title: mode === "visitor" ? `查看 ${ledger.displayName}` : `加入 ${ledger.displayName}`,
  ...
  deleteLedger() {
    const ledger = this.data.ledger;
    if (!ledger || !ledger.isOwner || this.data.deleting) return;
    wx.showModal({
      title: "删除账本",
      content: `确认删除「${ledger.displayName}」吗？其中的全部记录也会一并删除。`,
      confirmText: "删除",
      confirmColor: "#d9483b",
      success: async (res) => {
        ...
        try {
          const data = await ledgerService.deleteLedger(ledger.id);
