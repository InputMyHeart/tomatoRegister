...
Page({
  ...
  backToType() {
    if (!this.data.saving) this.setData({ stage: "type" });
  },
  onNameInput(event) {
  ...
  async createLedger() {
    ...
    const monthlyBudget = Number(this.data.monthlyBudget);
    if (Array.from(name).length < 1 || Array.from(name).length > 8) {
      wx.showToast({ title: "账本名称需为1-8个字符", icon: "none" });
      return;
    }
    if (
      this.data.budgetEnabled &&
      (!Number.isFinite(monthlyBudget) || monthlyBudget < 1 || monthlyBudget > 999999)
    ) {
      wx.showToast({ title: "预算金额需为1-999999", icon: "none" });
      return;
    }
    this.setData({ saving: true });
    try {
      const payload = await ledgerService.createLedger({
  ...
  goRecord() {
    wx.redirectTo({
      url: `/pages/record-category/index?ledgerId=${encodeURIComponent(this.data.createdLedgerId)}`,
    });
  },
  onShareAppMessage() {
