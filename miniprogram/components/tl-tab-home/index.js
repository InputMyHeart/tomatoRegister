...
Component({
  ...
  methods: {
    ...
    refreshDashboard() {
      if (this.data.isSwitchingLedger) return Promise.resolve(false);
    ...
    async chooseLedger(event) {
      ...
      if (!ledgerId || this.data.isSwitchingLedger) return;
      if (ledgerId === this.data.ledgerId) return;

      const ledger = this.data.ledgers.find((item) => (item._id || item.id) === ledgerId);
      if (!ledger) return;

      const previousData = { ...this.data };
      const previousLedger = app.globalData.currentLedger;
      const previousReadonly = app.globalData.readonly;
      const previousLedgerId = previousLedger && previousLedger._id;
      const previousUserLedgerId = app.globalData.user && app.globalData.user.currentLedgerId;
      let serverLedgerChanged = false;
      ...
      try {
        ...
        if (!saved) throw new Error("\u5207\u6362\u5931\u8d25");
        serverLedgerChanged = true;
        ...
        const loaded = await this.loadDashboard(ledgerId);
        if (!loaded) throw new Error("\u8d26\u672c\u6570\u636e\u52a0\u8f7d\u5931\u8d25");
        this.setData({ isSwitchingLedger: false });
      } catch (error) {
        console.warn("switch ledger failed", error);
        if (serverLedgerChanged && previousLedgerId) {
          try {
            await ledgerService.setCurrentLedger(previousLedgerId);
          } catch (rollbackError) {
            console.warn("rollback ledger switch failed", rollbackError);
          }
        }
        app.globalData.currentLedger = previousLedger;
        app.globalData.readonly = previousReadonly;
        if (app.globalData.user) app.globalData.user.currentLedgerId = previousUserLedgerId;
        app.persistAuthState();
        this.setData({
          ...previousData,
          isLedgerSwitcherVisible: false,
    ...
    goRecords() {
      this.triggerEvent("switchtab", { tab: "records" });
    },
